...
- `envDataType`: *DEPRECATED:* Only "string" is supported, and need not be specified in the config
  (was: string, either `file` or `sim`)

- The following entries are required, unless a default value or `optional` is specified:
  ...
  - *DEPRECATED:* if `envDataType` is "sim", the following entries are expected:
      - `mapParams`: *deprecated*; A subgroup of parameters containing the following keys:
          - `m`: *deprecated*; int, the number of distributary nodes per grid row/column
          - `n`: *deprecated*; int, the total number of nodes per grid row/column
          - `a`: *deprecated*; float, the distance between grid nodes in meters
          - `pBlind`: *deprecated*; float between 0 and 1, the probability of deleting a blind channel
          - `pDist`: *deprecated*; float between 0 and 1, the probability of deleting a distributary channel
      - `recruitSizeMean`: *deprecated*; float, the mean recruit size in millimeters
      - `recruitSizeStd`: *deprecated*; float, the standard deviation of recruit sizes in millimeters
      - `recruitRate`: *deprecated*; int, the average number of recruits per timestep
      - `simLength`: *deprecated*; int, the number of timesteps of environmental data to generate